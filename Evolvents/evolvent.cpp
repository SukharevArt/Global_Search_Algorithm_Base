...
// ------------------------------------------------------------------------------------------------
void TSmoothEvolvent::GetImage(const double& x, double* _y, int EvolventNum)
{
  if ((x < 0) || (x > 1))
  {
    throw ("x is out of range");
...
// ------------------------------------------------------------------------------------------------
void TSmoothEvolvent::GetInverseImage(double* _y, double& x)
{
  throw ("This method is not implemented for the smooth evolvent");
}
...
{
// ------------------------------------------------------------------------------------------------
  void numbr(double *iss, const int n1, const double& nexp,
             int& l, int* iu, int* iv,
             const double& extOne,
             const double& extZero,
             const double& extHalf)
  {
    /* calculate s(u)=is,l(u)=l,v(u)=iv by u=iu */
...
// ------------------------------------------------------------------------------------------------
  void node(double is, int n1, double nexp, int& l, int& iq, int iu[], int iv[],
            const double& extOne,
            const double& extZero,
            const double& extHalf)
  {
            ...
            const double& extHalf)
  {
    ...
    for (int j = 0; j < m; j++)
    {
      ...
      else
      {
        d = d * nexp;
        is = floor(d);
        //is = (int)d; //опасное преобразование при n > 32
        d = d - is;
      }
...
// ------------------------------------------------------------------------------------------------
void TLinearEvolvent::GetImage(const double& x, double* _y, int EvolventNum)
{
  if ((x < 0) || (x > 1))
  {
    throw ("x is out of range");
...
// ------------------------------------------------------------------------------------------------
void TLinearEvolvent::GetInverseImage(double* _y, double& x)
{
  // y ---> x
  memcpy(y, _y, N * sizeof(double));
  transform_D_to_P();
...
// ------------------------------------------------------------------------------------------------
void TNoninjectiveEvolvent::GetImage(const double& x, double* _y, int EvolventNum)
{
  if ((x < 0) || (x > 1))
  {
    throw ("x is out of range");
...
// ------------------------------------------------------------------------------------------------
int TNoninjectiveEvolvent::GetNoninjectivePreimages(double* _y, double* x)
{
  memcpy(y, _y, N * sizeof(double));
  transform_D_to_P();
  int preimNumber = 1;
